...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var RATIO_ALPHA = 0.4;
    /**
     * Floor on the visible-text share of a step's completion tokens. Below this
     * the step is dominated by reasoning/tool-call tokens the gauge never
     * streams, so calibrating against it would inflate the chars-per-token
     * ratio and bias the live estimate high. Steps below the floor are skipped.
     */
    var MIN_TEXT_SHARE = 0.5;
    ...
    /**
     * Read one non-negative token count off a usage object, guarded like the
     * shipped UI. Returns null when the field is absent or not a clean number.
     * @param usage - the node's usage object.
     * @param field - the usage field to read.
    ...
    function outputTokensOf(usage) {
      return usageCount(usage, 'outputTokens');
    }
    ...
    /**
     * Cache-hit percentage: the share of the step's prompt tokens served from
     * the provider's prompt cache. Mirrors the shipped chat UI's definition
     * (cacheRead / (total - output), i.e. the prompt side of the request),
     * with the same "never round a partial hit up to 100" honesty.
     * @param cacheRead - prompt tokens read from cache.
     * @param promptTokens - total prompt tokens (uncached + cached + written).
     * @returns percentage text, or null when the provider reported no prompt.
     */
    function cacheHitPercent(cacheRead, promptTokens) {
      if (typeof cacheRead !== 'number' || typeof promptTokens !== 'number' || promptTokens <= 0) return null;
      var missed = promptTokens - cacheRead;
      if (missed <= 0) return '100';
      var pct = (cacheRead / promptTokens) * 100;
     ...
     *    against settled steps that are *text-dominated*, so the estimate
     *    converges on the real tokenizer behaviour of whatever model is
     *    answering without being inflated by steps whose completion tokens are
     *    mostly reasoning or tool-call arguments the gauge never streams.
     */
    function createSampler() {
      ...
      return {
        ...
        settle: function settle(node) {
          ...
          // Calibrate only against steps whose completion tokens are
          // dominated by visible text. `usage.outputTokens` counts *all*
          // completion tokens (text + reasoning + tool-call arguments), while
          // `chars` counts only the streamed visible text. A step heavy in
          // reasoning or tool-call output therefore yields a ratio far above
          // the real text-tokenizer behaviour and would bias the live estimate
          // high. Skip such steps rather than letting them poison the ratio.
          if (chars / tokens < MIN_TEXT_SHARE) return;
      ...
      return function TokenSpeedWidget(props) {
        ...
        if (snapshot) {
          ...
          if (lastNode) {
            ...
            var usage = lastNode.usage;
            var cacheRead = usageCount(usage, 'cacheReadTokens');
            var cacheWrite = usageCount(usage, 'cacheWriteTokens');
            var reasoning = usageCount(usage, 'reasoningTokens');
            var inputTokens = usageCount(usage, 'inputTokens');
            var totalTokens = usageCount(usage, 'totalTokens');
            var promptTokens = null;
            if (totalTokens !== null && usageCount(usage, 'outputTokens') !== null) {
              promptTokens = totalTokens - usageCount(usage, 'outputTokens');
            }
            // Model label: the last route of the step, when the provider
            // reported it (provider/model).
            var modelLabel = null;
            var routes = usage && usage.routes;
            if (routes && routes.length > 0) {
              var lastRoute = routes[routes.length - 1];
              if (lastRoute && (lastRoute.provider || lastRoute.model)) {
                modelLabel = (lastRoute.provider || '') + (lastRoute.model ? '/' + lastRoute.model : '');
              }
            }
            settled = {
              tps: exactStepTps(lastNode),
              tokens: outputTokensOf(usage),
              ttftMs:
              ...
              cacheHit: cacheHitPercent(cacheRead, promptTokens),
              model: modelLabel,
            };
          }
          if (partial) {
        ...
        var onCopy = useCallback(
          function () {
            ...
            lines.push('缓存命中率: ' + (settled && settled.cacheHit !== null ? settled.cacheHit + '%' : '—'));
            lines.push('缓存读取: ' + (settled && settled.cacheRead !== null ? formatCount(settled.cacheRead) + ' tok' : '—'));
            lines.push('缓存写入: ' + (settled && settled.cacheWrite !== null ? formatCount(settled.cacheWrite) + ' tok' : '—'));
            lines.push('推理 token: ' + (settled && settled.reasoning !== null ? formatCount(settled.reasoning) + ' tok' : '—'));
            lines.push('输入 token: ' + (settled && settled.inputTokens !== null ? formatCount(settled.inputTokens) + ' tok' : '—'));
            lines.push('模型: ' + (settled && settled.model ? settled.model : '—'));
          ...
          open
            ? h(
                ...
                h(
                  ...
                  row('缓存命中率', settled && settled.cacheHit !== null ? settled.cacheHit + '%' : '—'),
                  row('缓存读取', settled && settled.cacheRead !== null ? formatCount(settled.cacheRead) + ' tok' : '—'),
                  row('缓存写入', settled && settled.cacheWrite !== null ? formatCount(settled.cacheWrite) + ' tok' : '—'),
                  row('推理 token', settled && settled.reasoning !== null ? formatCount(settled.reasoning) + ' tok' : '—'),
                  row('输入 token', settled && settled.inputTokens !== null ? formatCount(settled.inputTokens) + ' tok' : '—'),
                  row('模型', settled && settled.model ? settled.model : '—'),
